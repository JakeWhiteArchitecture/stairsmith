...
def _preview_single_winder(p):
    ...
    if actual_winders == 0:
        ...
        if turn_dir == "left":
            ...
            landing_cx = (width + ext) / 2
        # Landing nosing centered on newel post (corner_y + hp). Landing extends
        # backward by nosing and forward by width. Nosing IS the center of the box.
        landing_nosing = corner_y + hp
        landing_cy = landing_nosing
        landing_depth = nosing + width
        meshes.append(_box_mesh(
            landing_cx,
            landing_cy,
            landing_z + tread_t / 2,
...
def _preview_double_winder(p):
    ...
    if actual_winders1 == 0:
        ...
        if turn1_dir == "left":
            ...
            landing1_cx = (width + ext) / 2
        # Landing nosing centered on newel post (corner1_y + hp). Landing extends
        # backward by nosing and forward by width. Nosing IS the center of the box.
        landing1_nosing = corner1_y + hp
        landing1_cy = landing1_nosing
        landing1_depth = nosing + width
        meshes.append(_box_mesh(
            landing1_cx,
            landing1_cy,
